...
class DealsWriter(ResultWriter):
    ...
    def write(self, data, file_name=None, user_values=None, object_from_arrays=False, write_header=True):
        ...
        if d_stage_history and str(d_stage_history) != "nan" and len(d_stage_history) > 0:
            self.deals_stage_history_wr.write_all(
                d_stage_history, user_values={"Deal_ID": self._get_pkey_values(data, {})}
            )
